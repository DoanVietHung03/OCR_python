...
def process_single_frame(img_bgr,
    ...
    if frame_id % SKIP_FRAMES == 0:
        ...
        for i in range(len(tracked_detections)):
            ...
            # Khối lệnh OCR được chạy luôn ở frame nhận diện AI thay vì chờ frame_id % 3
            if needs_ocr:
        ...
        # Lưu lại danh sách vẽ và ID đang hoạt động vào bộ đệm (Cache) cho frame sau
        tracker_state['draw_list'] = draw_list
        tracker_state['active_ids'] = active_track_ids

        stale_ids = [tid for tid in ocr_cache.keys() if tid not in active_track_ids]
        for tid in stale_ids: del ocr_cache[tid]

    else:
        draw_list = tracker_state.get('draw_list', [])
