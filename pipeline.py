...
def process_single_frame(img_bgr,
    ...
    # BƯỚC 1: Cập nhật Object Detection (Xe) trên mọi frame
    ...
    # BƯỚC 2: Cập nhật Tracker trên mọi frame để mượt ID
    ...
    for i in range(len(tracked_detections)):
        ...
        # Kiểm tra Cache xem ID này đã đủ độ tự tin chưa
        ...
        # BƯỚC 3: Xử lý OCR (Chỉ kích hoạt mỗi 3 frame để giảm tải GPU)
            ...
            if vehicle_roi.size > 0:
                ...
                for p_det in plates:
                    ...
                    if ratio > 2.2:
                        ...
                        if not re.match(pattern, current_text): holistic_score *= 0.70
        ...
        if track_id in ocr_cache:
            ...
            if res.confidence >= 0.65:
                ...
                # Chỉ xuất log CSV và lưu hình ảnh 1 lần cho mỗi xe
