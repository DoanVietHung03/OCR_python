...
def process_video(input_path, output_path):
    ...
    tracker = sv.ByteTrack(track_activation_threshold=0.5, lost_track_buffer=60, minimum_matching_threshold=0.8, frame_rate=fps)
    ocr_cache = {}
    tracker_state = {}
    frame_id = 0

    print(f"⚙️ Bắt đầu xử lý! Tổng số frame: {total_frames} | Độ phân giải: {width}x{height} | FPS gốc: {fps}")
    ...
    while True:
        ...
        process_single_frame(
            ...
            allowed_vehicle_ids,
            tracker, ocr_cache, tracker_state, frame_id=frame_id,
        )
        ...
        if frame_id % 30 == 0:
            ...
            print(f" ➜ Đã xử lý: {frame_id}/{total_frames} frames ({percent:.1f}%) - Tốc độ: {process_fps:.1f} FPS")
