...
- Điểm tự tin của toàn bộ biển số được tính bằng công thức thiên vị ký tự yếu nhất: (Điểm trung bình _ 0.4) + (Điểm thấp nhất _ 0.6).
...
# 7. Tối ưu hóa tốc độ ONNX Runtime (Optimization)

Mô hình PARSeq có thể chạy chậm nếu chỉ load bằng cấu hình mặc định. Để đạt hiệu năng cao nhất, khi khởi tạo InferenceSession, bắt buộc phải cấu hình các thông số sau:

- Bật tối ưu hóa đồ thị (Graph Optimization): Set graph_optimization_level = ORT_ENABLE_ALL để ONNX tự động gộp các node tính toán.

- Cấu hình đa luồng (Threading): Giới hạn intra_op_num_threads = 4 và inter_op_num_threads = 4 để tránh hiện tượng CPU bị quá tải (bottleneck) khi xử lý các node không được hỗ trợ trên GPU.

- Tối ưu bộ nhớ CUDA (NVIDIA GPU): Khi khai báo CUDAExecutionProvider, cần truyền thêm các tham số:
  - "cudnn_conv_algo_search": "HEURISTIC": Giúp tìm thuật toán tích chập tối ưu nhất.

  - "cudnn_conv_use_max_workspace": "1".

  - "arena_extend_strategy": "kNextPowerOfTwo": Tránh phân mảnh VRAM.
...
session = ort.InferenceSession("weights/parseq.onnx", sess_options, providers=providers)
```